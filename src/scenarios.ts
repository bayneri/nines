...
const SOURCES: Scenario[] = [
  {
    ...
    more: 'Napkin math treats the two regions as independent and says 99.990% of requests succeed. The model says 99.939%: the control plane fails by outage, which takes out both regions at once, so the pair can never be more available than it is. Redundancy only multiplies away failures that are independent.',
    tryIt: {
      label: 'Make control-plane failures flaky',
      apply: (doc) => updateNode(doc, 'control_plane', { transient: 1 }),
      result: 'Now the regions really do cover for each other: 99.989% succeed, next to napkin math’s 99.990%. The promise is still broken, and what’s left is time.',
      more: 'Flaky failures are independent from one request to the next, so a failure behind one region rarely lines up with one behind the other. Same 99.95% on paper, a very different way of failing. The remaining gap is failover: a request that falls back to the farther EU West region can take longer than 500 ms.',
    },
    dot: multiRegionDot,
  ...
  },
  {
    ...
    more: 'Ignoring time, napkin math is right: 98.94% of requests succeed. But a shard is slower than its 300 ms timeout about once in 600 calls, and with 100 shards per request that happens to about 1 request in 6, which then fails. The p99 of successful requests still looks healthy, because the slow requests became failures instead.',
    tryIt: {
      label: 'Tolerate 5 missing shards',
      apply: (doc) => updateCall(doc, callIndex(doc, 'search_api', 'shard'), { fanoutRequire: 95 }),
      result: 'Success within 400 ms jumps from 83.4% to 99.85%, but those answers are now partial.',
  ...
  },
  {
    ...
    more: 'Payments fails mostly in flaky ways, so three retries recover it. The ledger fails by outage, which lasts through every retry. The retries also add latency: 99.9% availability and a p99 under 800 ms both hold, yet only 99.26% of requests succeed within 800 ms.',
    tryIt: {
      label: 'Remove the retries',
      apply: (doc) => updateCall(doc, callIndex(doc, 'checkout', 'payments'), { retries: 0 }),
      result: 'Succeeding at all drops from 99.930% to 99.20%: slow payments calls now fail at the 800 ms timeout.',
  ...
  },
  {
    ...
    more: 'Inventory is required only because the page shows stock. If the page can load without it, success within 300 ms rises from 99.64% to 99.829%; making inventory 99.99% available only gets to 99.73%. Napkin math is pessimistic here: it counts the shared auth service three times, once per caller.',
    tryIt: {
      label: 'Make inventory optional',
      apply: (doc) => updateCall(doc, callIndex(doc, 'product_page', 'inventory'), { dependency: 'soft', timeoutMs: 150 }),
      result: 'Success within 300 ms rises from 99.64% to 99.829%. The price: more pages load without stock information.',
      more: 'Complete answers within 300 ms drop from 95.5% to 94.5%. An optional call trades completeness for availability.',
    },
    dot: productPageDot,
  ...
  },
  {
    ...
    summary: 'Sales promised 99.99% of requests within 300 ms. Today 99.64% make it.',
    more: 'That’s about 1.5 nines short. The best single change, making inventory optional, reaches 99.829%. Getting to 99.99% takes several changes at once (“Find a path to the promise” shows which), or a different promise.',
    tryIt: {
